...
#     "Website": "https://metroairport.diversitycompliance.com/FrontEnd/searchcertifieddirectory.asp",
#     "Title": "Certified Vendor Directory",
#     "Download": "Manual",
#     "FileName": "MI_WCAA.csv",
#   }
#
...
def processFile(filename):
    ...
    with open(filename) as f:
        ...
        for row_num, row in enumerate(reader):
            ...
            dict1['website'] = prepare_csv_data(row[12])
            dict1['phone'] = prepare_csv_data(row[10])
            dict1['email'] = prepare_csv_data(row[11])
            dict1['gdiverse'] = prepare_csv_data(row[14])
            dict1['cert_agency'] = prepare_csv_data(row[13])
            dict1['cert_num'] = prepare_csv_data(row[16])
            (pnaics, onaics) = get_pnaics_onaics(row[17])
            dict1['pnaics'] = pnaics
            ...
            dict1['addl5'] = prepare_csv_data(row[15], prefix='Capabilities')
