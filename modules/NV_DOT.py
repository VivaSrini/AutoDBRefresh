...
def processFile(filename):
    ...
    with open(filename) as f:
        ...
        for row_num, row in enumerate(reader):
            ...
            dict1['cert_agency'] = prepare_csv_data(row[24])
            ...
            dict1['addl5'] = prepare_csv_data(row[20], prefix='Certified')
            dict1['addl6'] = prepare_csv_data(row[21], prefix='Capabilities')
            dict1['addl7'] = prepare_csv_data(row[23], prefix='Work Districts/Regions')
            dict1['addl8'] = prepare_csv_data(row[22], prefix='County')
            dict1['addl9'] = prepare_csv_data(row[25], prefix='Business Type')
